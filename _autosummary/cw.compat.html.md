...
those 27 call sites already do. [`cw.mk_parser()`](cw.html.md#cw.mk_parser) returns a plain
...
The values are cw’s [`cw.Convention`](cw.html.md#cw.Convention) `naming` strings, so a policy passed here
...
it travels in the leaf and [`cw.cli`](cw.cli.html.md#module-cw.cli) assigns it to the action it creates.
