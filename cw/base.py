...
import dataclasses
import inspect
from typing import Any, Callable, Container, Mapping, TypeAlias, Union
...
class _Sentinel:
    """A named singleton whose ``repr`` is useful in a traceback or a help string.

    >>> _Sentinel('cw.NOPE')
    cw.NOPE
    >>> bool(_Sentinel('cw.NOPE'))
    True
    """
...
#: :func:`functools.partial` whose pre-bound keyword :func:`inspect.signature` still
...
HIDE = _Sentinel("cw.HIDE")


# --------------------------------------------------------------------------------------
# Seam contracts
#
# Named here so that cw, and anyone replacing one of cw's defaults, spell them the same way.

#: Seam 1. ``(parameter, hint) -> add_argument kwargs``.
#:
#: Given a :class:`inspect.Parameter` and its type hint (whatever
...
Decode: TypeAlias = Callable[
    [inspect.Parameter, Any], Union[None, Callable[[str], Any], Mapping[str, Any]]
]

#: Seam 2. ``(result, *, out, err) -> exit code``.
#:
#: How a function's return value becomes lines on a stream and a process exit code.
Egress: TypeAlias = Callable[..., int]


# --------------------------------------------------------------------------------------
# Errors

#: The exit code a :class:`CommandError` uses when none is given.
DFLT_ERROR_CODE = 1
...
class CommandError(Exception):
    ...
    Args:
        message: The single line written to the error stream.
        code: The process exit code. Keyword-only.
    ...
    def __init__(self, message: str = "", *, code: int = DFLT_ERROR_CODE):
        self.code = code
        super().__init__(message)


# --------------------------------------------------------------------------------------
# The ingress-site codec
...
class Codec:
    ...
    **opt-in**, per-parameter site, which runs after parsing.
    ...
    Args:
        decode: ``value -> value``. Called only on strings.
        passthrough: Values ``decode`` must never see, so a sentinel can survive.
    ...
    def __call__(self, value: Any) -> Any:
        if isinstance(value, str):
            return value if value in self.passthrough else self.decode(value)
...
class ArghHelpFormatter(
    ...
    render_default = staticmethod(repr)

    def _help_params(self, action: argparse.Action) -> dict:
        """The ``%(...)s`` substitution namespace for one action's help string.

        Separated from :meth:`_expand_help` so the *rendering* rules can be read, tested,
        and overridden without touching argparse's interpolation call.
        """
        ...
        params = {
            name: getattr(value, "__name__", value) for name, value in params.items()
        }
        if params.get("choices") is not None:
            params["choices"] = self.CHOICES_SEPARATOR.join(
                str(choice) for choice in params["choices"]
            )
        if "default" in params:
            default = params["default"]
            params["default"] = (
                self.NONE_IN_HELP if default is None else self.render_default(default)
            )
        return params

    def _expand_help(self, action: argparse.Action) -> str:
        return self._get_help_string(action) % self._help_params(action)
